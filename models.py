...
class GeneratorCNN(BaseModel):
    def __init__(self, z_num, initial_conv_dim, output_num, repeat_num, hidden_num, num_gpu):
        super(GeneratorCNN, self).__init__()
        self.num_gpu = num_gpu
        layers = []

        self.initial_conv_dim = initial_conv_dim
        self.fc = nn.Linear(z_num, int(np.prod(self.initial_conv_dim)))
...
class DiscriminatorCNN(BaseModel):
    def __init__(self, input_channel, z_num, repeat_num, hidden_num, num_gpu):
        ...
        self.fc2 = nn.Linear(z_num, int(np.prod(self.conv2_input_dim)))
    ...
    def main(self, x):
        # Encoder
        conv1_out = self.conv1(x).view(-1, int(np.prod(self.conv1_output_dim)))
